...
class AnalyticController extends Controller
{
    ...
    protected $searched_data;
    protected $customer;
    protected $searched_keyword_count;
    protected $searched_category;

    public function __construct(RecentSearch $recent_search, SearchedData $searched_data, User $customer, SearchedKeywordCount $searched_keyword_count, SearchedCategory $searched_category)
    {
        $this->recent_search = $recent_search;
        $this->searched_data = $searched_data;
        $this->customer = $customer;
        $this->searched_keyword_count = $searched_keyword_count;
        $this->searched_category = $searched_category;
    }

    public function get_keyword_search(Request $request)
    {
    ...
    public function get_customer_search(Request $request)
    {
        ...
        //Top customers
        $top_customer = SearchedKeywordUser::with(['customer'])
            ->when($request->has('date_range'), function ($query) use ($request) {
                //DATE RANGE
                if ($request['date_range'] == 'today') {
                    // today's data
        ...
        foreach ($top_customer as $item) {
            $graph_data['top_customers'][] = $item->customer->f_name . ' ' . $item->customer->l_name;
            $graph_data['search_volume'][] = $item->count;
        }

        //Top Products
        $top_products = SearchedProduct::
            when($request->has('date_range_2'), function ($query) use ($request) {
                //DATE RANGE
                if ($request['date_range_2'] == 'today') {
                    // today's data
        ...
        $customers_data = SearchedKeywordUser::with(['customer', 'related_category', 'related_product', 'related_category.category'])
            ->when($request->has('search'), function ($query) use ($request) {
                $keys = explode(' ', $request['search']);
    ...
    public function export_keyword_search(Request $request)
    {
    ...
    public function export_customer_search(Request $request)
    {
        $customers_list = SearchedKeywordUser::with(['customer', 'related_category', 'related_product', 'related_category.category'])
            ->select('user_id', DB::raw('count(*) as search_volume'))
            ->whereNotNull('user_id')
            ->groupBy('user_id')
            ->withCount('visited_products', 'orders', 'related_category', 'related_product')
