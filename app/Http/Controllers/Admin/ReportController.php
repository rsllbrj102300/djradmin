...
class ReportController extends Controller
{
    ...
    public function expense_index(Request $request)
    {
        ...
        $expense_calculate = Order::with('coupon')
        ...
        $expense_transactions_table = Order::with('coupon')
            ->where('order_status', 'delivered')
            ->where(function ($query){
                $query->whereNotIn('coupon_code', ['0', 'NULL'])
                    ->orWhere('free_delivery_amount', '>', 0)
    ...
    public function expense_summary_pdf(Request $request)
    {
        ...
        $company_logo = BusinessSetting::where('key', 'logo')->first()->value ?? '';
        ...
        $expense_report = Order::with('coupon')
            ->where('order_status', 'delivered')
            ->where(function ($query){
                $query->whereNotIn('coupon_code', ['0', 'NULL'])
                    ->orWhere('free_delivery_amount', '>', 0)
    ...
    public function expense_export_excel(Request $request)
    {
        $search = $request['search'];
        $from = $request['start_date'];
        $to = $request['end_date'];
